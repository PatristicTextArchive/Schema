...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt">
   ...
   <!-- This file generated 2026-06-08T14:28:16Z by 'extract-isosch.xsl'. -->
   <!-- ********************* -->
   <!-- namespaces, implicit: -->
   <!-- ********************* -->
   ...
   <pattern id="schematron-constraint-encodingDesc-5">
      <rule context="tei:encodingDesc">
         <assert test="child::tei:refsDecl[@n='CTS'] and child::tei:refsDecl[@xml:id='biblical'] and child::tei:refsDecl[@xml:id='pta'] and child::tei:refsDecl[@xml:id='perseus'] and child::tei:refsDecl[@xml:id='cllg'] and child::tei:editorialDecl and child::tei:classDecl and child::tei:schemaRef"> Needs refDecl for CTS, biblical, PTA, Perseus library and CLLG references, editorialDecl, classDecl and schemaRef</assert>
      </rule>
   </pattern>
   ...
   <pattern id="schematron-constraint-bibl-12">
      <rule context="tei:bibl">
         <report test="current()[preceding-sibling::tei:listBibl or following-sibling::tei:listBibl]">
            <name/> needs to be child of a listBibl (sibling to the following or preceding listBibl)</report>
      </rule>
   </pattern>
   <pattern id="schematron-constraint-biblStruct-13">
      <rule context="tei:biblStruct">
         <report test="current()[preceding-sibling::tei:listBibl or following-sibling::tei:listBibl]">
            <name/> needs to be child of a listBibl (sibling to the following or preceding listBibl)</report>
      </rule>
   </pattern>
   <pattern id="schematron-constraint-emph-14">
      <rule context="tei:emph">
         <report test="not(ancestor::tei:div[@type='praefatio' or @type='translation' or @type='commentary'])">
            <name/> may only be used in the praefatio of an edition, in a translation or in a commentary</report>
      </rule>
   </pattern>
   <pattern id="schematron-constraint-label-15">
      <rule context="tei:label">
         <report test="@type='attribution' and not(ancestor::tei:div[@type='edition'])">attribution is only to be used in editions</report>
         <report test="@type='lemma' and not(ancestor::tei:div[@type='commentary'])">commentary is only to be used in commentaries</report>
      </rule>
   </pattern>
   ...
   <pattern id="schematron-constraint-unclear-16">
   ...
   <pattern id="schematron-constraint-gap-17">
   ...
   <pattern id="schematron-constraint-ref-18">
      <rule context="tei:ref">
         ...
         <report test="@cRef and not(@decls='#biblical' or @decls='#pta' or @decls='#perseus' or @decls='#cllg')">if <name/> has @cRef, it also needs @decls (#biblical, #pta, #perseus or #cllg)</report>
         <report test="(@decls='#pta' or @decls='#perseus' or @decls='#cllg') and not(starts-with(@cRef, 'urn:cts:'))">Check that a reference to PTA or Perseus library is an URN</report>
         <report test="current()[ancestor::tei:div[@type='edition' or @type='translation']]/text() and not(parent::tei:cit or @target)">
   ...
   <pattern id="schematron-constraint-note-19">
      <rule context="tei:note">
         ...
         <report test="@corresp and not(parent::tei:app)">@corresp may only be used in a note in an apparatus.</report>
         ...
         <report test="ancestor::tei:div[@type='praefatio'] and (@type or @resp or @corresp)">Notes in the praefatio are not allowed to have attributes.</report>
   ...
   <pattern id="schematron-constraint-del-20">
   ...
   <pattern id="schematron-constraint-abbr-21">
   ...
   <pattern id="schematron-constraint-msIdentifier-24">
   ...
   <pattern id="schematron-constraint-origDate-25">
   ...
   <pattern id="schematron-constraint-space-26">
   ...
   <pattern id="schematron-constraint-rdg-30">
      <rule context="tei:rdg">
         ...
         <report test="parent::tei:app[@type='witnesses'] and @resp and not(@wit or @source)">resp is not allowed in witnesses attestation, only source or wit</report>
